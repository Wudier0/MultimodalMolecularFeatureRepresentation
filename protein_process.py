...
def Protein_embedding_process(dataset, fold, dir_output, id_train, id_test):
    ...
    max_shape = max([embedding.shape for embedding in proteins_embedding_train], key=lambda x: np.prod(x))
    padded_embeddings = [np.pad(embedding, [(0, max_shape[i] - embedding.shape[i]) for i in range(len(embedding.shape))]) 
                        for embedding in proteins_embedding_train]
    proteins_embedding_train = np.array(padded_embeddings)
    ...
    max_shape = max([embedding.shape for embedding in proteins_embedding_test], key=lambda x: np.prod(x))
    padded_embeddings = [np.pad(embedding, [(0, max_shape[i] - embedding.shape[i]) for i in range(len(embedding.shape))]) 
                        for embedding in proteins_embedding_test]
    proteins_embedding_test = np.array(padded_embeddings)
